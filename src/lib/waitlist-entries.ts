...
/** Base URL for browser `fetch` when using `output: "export"` (no `/api/waitlist` proxy). */
export function getWaitlistApiBaseUrlForBrowser(): string {
  return process.env.NEXT_PUBLIC_WAITLIST_API_URL || DEFAULT_ENTRIES_URL;
}
